...
import 'containers/CandidateAccountManager/CandidateAccountManager.less';
...
export default function CandidateAccountManager() {
  ...
  const editItem = (val) => {
    navigate(`/candidateaccountedit/${val}`, { replace: false });
  };

  const getAccountData = async () => {
    let accounts = await fetch(
      'http://localhost:3001/adminUmat/getAccountCandidateList',
      {
        method: 'GET',
  ...
  const addNew = () => {
    navigate('/candidateaccount/createedit');
  };
  ...
  return (
    <div className="list-container">
      <Row style={{ marginBottom: '6.17px' }}>
        <FormHeader
          title={'Danh sách tài khoản ứng viên'}
